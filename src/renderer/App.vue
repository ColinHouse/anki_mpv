<template>
  ...
  <div class="app-container h-screen w-screen overflow-hidden rounded-xl border border-white/50 shadow-2xl flex flex-col relative bg-transparent">
    
    <!-- Custom Title Bar -->
    <div class="window-drag-handle w-full h-8 flex justify-between items-center px-4 bg-white/40 backdrop-blur-md z-50 shrink-0 border-b border-white/10 select-none">
      <div class="flex items-center gap-2">
         <span class="text-xs text-gray-700 font-bold tracking-wider">Anki-MPV AI Studio</span>
      </div>
      ...
      <div class="flex items-center gap-3 no-drag">
        ...
        <button @click="minWindow" class="p-1 hover:bg-black/5 rounded group" title="最小化">
           <svg class="w-3 h-3 text-gray-500 group-hover:text-gray-900" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4"></path></svg>
        </button>
        <button @click="maxWindow" class="p-1 hover:bg-black/5 rounded group" title="最大化">
           <svg class="w-3 h-3 text-gray-500 group-hover:text-gray-900" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 8V4m0 0h4M4 4l5 5m11-1V4m0 0h-4m4 0l-5 5M4 16v4m0 0h4m-4 0l5-5m11 5l-5-5m5 5v-4m0 4h-4"></path></svg>
        </button>
        <button @click="closeWindow" class="p-1 hover:bg-red-500 hover:text-white rounded group" title="关闭">
           <svg class="w-3 h-3 text-gray-500 group-hover:text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
        </button>
      </div>
    </div>

    <!-- Main Content Area (Glassy Background) -->
    <div class="flex-1 flex overflow-hidden relative bg-white/80 backdrop-blur-2xl">
      <!-- 侧边栏：任务队列 -->
      <SidebarQueue 
    ...
    >
      <div class="bg-white rounded-2xl shadow-2xl p-8 flex flex-col items-center max-w-sm w-full animate-in fade-in zoom-in duration-300">
        <!-- 动画图标 -->
        <div class="relative w-20 h-20 mb-6">
          <div class="absolute inset-0 border-4 border-blue-100 rounded-full animate-ping opacity-75"></div>
        ...
        <div class="text-gray-500 text-center text-sm mb-6">
           <div class="text-sm text-gray-700">正在处理...</div><br/>
          请勿关闭窗口...
        </div>
...
const { videoQueue, selectedFilePath, currentVideoPath, selectedFile, loadVideoFromQueue } = useVideoQueue();
...
watch(currentVideoPath, async (newPath) => {
  if (newPath && serverPort.value) {
    ...
    if (item) {
        ...
        try {
           const exists = await (window as any).api.invoke("check-file-exists", potentialSrt);
           if (exists) {
              const content = await (window as any).api.invoke("read-file", potentialSrt);
              if (content) {
